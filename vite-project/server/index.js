...
// Cloudinary storage for notes (documents: pdf, docx, ppt)
const noteStorage = new CloudinaryStorage({
  cloudinary,
  params: {
    folder: 'codeworld/notes',
    resource_type: 'raw',
  },
});

// Cloudinary storage for lectures (videos)
const lectureStorage = new CloudinaryStorage({
  cloudinary,
  params: {
    folder: 'codeworld/lectures',
    resource_type: 'video',
  },
});
...
app.post('/upload/notes', uploadNote.single('file'), async (req,res)=>{
    const {subject,topic,description}=req.body;
    const fileUrl = req.file.path;

    try{
        const note=await Note.create({filename: fileUrl, subject, topic, description});
        res.json({status:"Success", note});
    }
    catch (err){
        res.status(500).json({ error: err.message });
...
app.post('/upload/lectures', uploadLecture.single('file'), async (req,res)=>{
    const {subject,topic,description}=req.body;
    const fileUrl = req.file.path;

    try{
        const lecture=await Lecture.create({filename: fileUrl, subject, topic, description});
        res.json({status:"Success", lecture});
    }
    catch (err){
        res.status(500).json({ error: err.message });
